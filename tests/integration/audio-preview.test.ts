...
beforeAll(async () => {
  storagePath = await mkdtemp(join(tmpdir(), "open-video-studio-audio-preview-"));
...
describe("audio preview routes (integration)", () => {
  it("persists selected voice on the project and returns a preview for a scene", async () => {
    ...
    await storage.putObject("audio", "voice-profiles/preview.wav", buildWavBuffer(3.4), "audio/wav");
