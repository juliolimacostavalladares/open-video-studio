...
const envSchema = z
  .object({
    ...
    DATABASE_URL: z
      ...
      .url()
      .default(
        'postgresql://postgres:postgres@localhost:5432/open_video_studio?schema=public',
      ),
  ...
  .superRefine((data, ctx) => {
    if (data.NODE_ENV === 'production') {
      if (
        ...
        data.DATABASE_URL.includes('postgres:postgres')
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message:
            'DATABASE_URL contains insecure default credentials or points to localhost in production mode.',
          path: ['DATABASE_URL'],
        });
      }
      if (data.MINIO_ROOT_PASSWORD === 'minioadmin') {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message:
            'MINIO_ROOT_PASSWORD cannot be the default "minioadmin" in production mode.',
      ...
      if (data.BULL_BOARD_PASSWORD === 'admin') {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message:
            'BULL_BOARD_PASSWORD cannot be the default "admin" in production mode.',
