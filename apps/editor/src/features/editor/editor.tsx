...
import { mapProjectScenesToDesign } from "./utils/project-mapper";
...
const Editor = ({ tempId, id }: { tempId?: string; id?: string }) => {
  ...
  const { timeline, playerRef, setProjectId, setVoiceProfileId } = useStore();
  ...
  const borderClass = "border-border/90";
  ...
  useEffect(() => {
    ...
    async function loadProjectData() {
      try {
        const pRes = await fetch(`${apiBaseUrl}/projects/${id}`);
        if (!pRes.ok) throw new Error("Failed to load project metadata");
        const project = await pRes.json();

        const sRes = await fetch(`${apiBaseUrl}/projects/${id}/scenes`);
        if (!sRes.ok) throw new Error("Failed to load project scenes");
        ...
        console.error("Error fetching project in Editor:", error);
      }
    }
